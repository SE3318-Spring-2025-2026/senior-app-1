...
export default function AdvisorFinalGradePage() {
    ...
    async function init() {
      try {
        ...
        if (gradesRes.status === 'fulfilled') {
          const ag = gradesRes.value.data.data?.advisorGrade;
          if (ag) setExistingGrade(ag);
        }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await apiClient[method](`/v1/final-evaluation/groups/${groupId}/advisor-grade`, body);
      setFeedback({
        type: 'success',
        message: `Advisor grade ${existingGrade ? 'updated' : 'submitted'}. Final score: ${(res.data.data.finalScore * 100).toFixed(1)} / 100`,
      });
      setExistingGrade(res.data.data);
      setSubmitted(true);
    } catch (err) {
  ...
  return (
    <div className="page">
      ...
      <div className="hero">
        <p className="eyebrow">Final Evaluation · Advisor Grade</p>
        <h1>Submit Advisor Grade</h1>
        <p className="subtitle" style={{ fontFamily: 'monospace', fontSize: '0.85rem' }}>Group: {groupId}</p>
      </div>

      {existingGrade && !submitted && (
        <div className="feedback feedback-idle" style={{ marginBottom: '16px' }}>
          <div className="feedback-label">existing grade</div>
          <p>You have already submitted an advisor grade (score: {(existingGrade.finalScore * 100).toFixed(1)} / 100). Submitting again will overwrite it.</p>
        </div>
      )}
        ...
        <form className="form" onSubmit={handleSubmit}>
          ...
          {criteria.length > 0 && deliverables.length > 0 && (
            <button type="submit" disabled={isSubmitting || submitted}>
              {isSubmitting ? 'Submitting...' : submitted ? 'Grade Submitted' : existingGrade ? 'Update Grade' : 'Submit Grade'}
            </button>
          )}
        </form>
      </div>
