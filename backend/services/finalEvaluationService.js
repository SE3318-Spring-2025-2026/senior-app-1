...
class FinalEvaluationService {
  ...
  const FinalEvaluationService = {
    // Committee grade submission
    async submitCommitteeGrade({ groupId, deliverableId, submittedBy, scores, comments }) {
      // Validation
      if (!groupId || typeof groupId !== 'string') throw serviceError('INVALID_GROUP_ID', 'Invalid group ID');
      if (!deliverableId || typeof deliverableId !== 'string') throw serviceError('INVALID_DELIVERABLE_ID', 'Invalid deliverable ID');
      if (!submittedBy) throw serviceError('INVALID_REVIEWER_ID', 'Reviewer user ID is required');
      ...
      if (group.status === 'FINALIZED') throw serviceError('FINALIZATION_LOCK_ERROR', 'Cannot submit grade after finalization');
      ...
      if (deliverable.groupId !== groupId) throw serviceError('DELIVERABLE_GROUP_MISMATCH', 'Deliverable does not belong to this group');
      const existing = await FinalEvaluationGrade.findOne({ where: { groupId, deliverableId, submittedBy } });
      if (existing) throw serviceError('DUPLICATE_REVIEWER_ERROR', 'This reviewer has already submitted a grade for this deliverable');
      const finalScore = scores.reduce((sum, s) => sum + (s.value || 0), 0) / scores.length;
      const grade = await FinalEvaluationGrade.create({ groupId, deliverableId, submittedBy, scores, comments: comments || null, finalScore });
      (async () => { try { await AuditLog.create({ groupId, action: 'COMMITTEE_GRADE_SUBMITTED', userId: submittedBy, details: { deliverableId, scores, comments } }); } catch {} })();
      return grade;
    },

    async updateCommitteeGrade({ groupId, deliverableId, submittedBy, scores, comments }) {
      if (!groupId || typeof groupId !== 'string') throw serviceError('INVALID_GROUP_ID', 'Invalid group ID');
      if (!deliverableId || typeof deliverableId !== 'string') throw serviceError('INVALID_DELIVERABLE_ID', 'Invalid deliverable ID');
      if (!submittedBy) throw serviceError('INVALID_REVIEWER_ID', 'Reviewer user ID is required');
      if (!Array.isArray(scores) || scores.length === 0) throw serviceError('INVALID_SCORES', 'At least one score is required');
      ...
      const group = await Group.findByPk(groupId);
      if (!group) throw serviceError('GROUP_NOT_FOUND', 'Group not found');
      if (group.status === 'FINALIZED') throw serviceError('FINALIZATION_LOCK_ERROR', 'Cannot update grade after finalization');
      const deliverable = await Deliverable.findByPk(deliverableId);
      if (!deliverable) throw serviceError('DELIVERABLE_NOT_FOUND', 'Deliverable not found');
      if (deliverable.groupId !== groupId) throw serviceError('DELIVERABLE_GROUP_MISMATCH', 'Deliverable does not belong to this group');
      const existing = await FinalEvaluationGrade.findOne({ where: { groupId, deliverableId, submittedBy } });
      ...
      await existing.save();
      (async () => { try { await AuditLog.create({ groupId, action: 'COMMITTEE_GRADE_UPDATED', userId: submittedBy, details: { deliverableId, scores, comments } }); } catch {} })();
      return existing;
    },

    // Advisor grade submission
    async submitAdvisorGrade({ groupId, deliverableId, advisorId, scores, comments }) {
      const group = await Group.findByPk(groupId);
      if (!group) throw serviceError('GROUP_NOT_FOUND', 'Group not found');
      ...
      (async () => { try { await AuditLog.create({ groupId, action: 'ADVISOR_GRADE_SUBMITTED', userId: advisorId, details: { deliverableId, scores, comments } }); } catch {} })();
      return grade;
    },

    async updateAdvisorGrade({ groupId, deliverableId, advisorId, scores, comments }) {
      const group = await Group.findByPk(groupId);
      if (!group) throw serviceError('GROUP_NOT_FOUND', 'Group not found');
      ...
      existing.finalScore = scores.reduce((sum, s) => sum + (s.value || 0), 0) / scores.length;
      await existing.save();
      (async () => { try { await AuditLog.create({ groupId, action: 'ADVISOR_GRADE_UPDATED', userId: advisorId, details: { deliverableId, scores, comments } }); } catch {} })();
      return existing;
    },
  };
