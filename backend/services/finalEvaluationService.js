...
// Committee grading logic (submitCommitteeGrade, updateCommitteeGrade, etc.)
// ... Paste the committee grading logic from the other branch here ...

module.exports = {
  calculateTeamScalar,
  getTeamScalar,
  getContributions,
  // ...export committee grading functions here...
};
/**
...
=======
async function submitAdvisorGrade({ groupId, userId, finalScore, scores, comments, deliverableId }) {
  const group = await Group.findByPk(groupId);
  if (!group) throw serviceError('GROUP_NOT_FOUND', 'Group not found');
  if (parseInt(group.advisorId) !== parseInt(userId)) {
    throw serviceError('FORBIDDEN', 'Only the assigned advisor can submit/update the grade for this group');
  }
  if (deliverableId) {
    const deliverable = await Deliverable.findByPk(deliverableId);
    if (!deliverable || deliverable.groupId !== groupId) {
      throw serviceError('FORBIDDEN', 'Deliverable does not exist or does not belong to this group');
    }
  }
  const [grade, created] = await FinalEvaluationGrade.upsert({
>>>>>>> 163acc7 (feat: add advisor grade endpoint, service, and validation (PR #366 requirements, conflict-free))
    groupId,
    deliverableId,
    submittedBy,
